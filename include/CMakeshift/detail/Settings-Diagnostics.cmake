...
function(_CMAKESHIFT_SETTINGS_DIAGNOSTICS)
    ...
    if(SETTING STREQUAL "diagnostics-pedantic")
        ...
        if(MSVC)
            ...
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/W4${RB}")
        elseif(CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX OR (CMAKE_CXX_COMPILER_ID MATCHES "Clang"))
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}-Wall${RB}" ${PASSTHROUGH} "${LB}-Wextra${RB}" ${PASSTHROUGH} "${LB}-pedantic${RB}")
        endif()

    elseif(SETTING STREQUAL "diagnostics-paranoid")
        # enable extra paranoid warnings
        if(MSVC)
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/w44062${RB}") # enumerator 'identifier' in a switch of enum 'enumeration' is not handled
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/w44242${RB}") # 'identifier': conversion from 'type1' to 'type2', possible loss of data
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/w44254${RB}") # 'operator': conversion from 'type1' to 'type2', possible loss of data
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/w44265${RB}") # 'class': class has virtual functions, but destructor is not virtual
            #target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/w44365${RB}") # 'action': conversion from 'type_1' to 'type_2', signed/unsigned mismatch (cannot enable this one because it flags `container[signed_index]`)
        endif()

    elseif(SETTING STREQUAL "fatal-errors")
        # every error is fatal; stop after reporting first error
        if(CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}-fmax-errors=1${RB}")
        elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}-ferror-limit=1${RB}")
        endif()
    ...
    elseif(SETTING STREQUAL "diagnostics-disable-annoying")
        ...
        if(MSVC)
            ...
            target_compile_options(${TARGET_NAME} ${SCOPE} ${PASSTHROUGH} "${LB}/wd4324${RB}")
