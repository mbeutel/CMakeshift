    ...
    if(SETTING STREQUAL "cpu-architecture")
        ...
        if(NOT ARCH STREQUAL "default" AND NOT ARCH STREQUAL "")
            ...
            if(MSVC)
                if(ARCH STREQUAL "penryn")
                    if(CMAKE_SIZEOF_VOID_P EQUAL 8) # compiling for x64
                        target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/favor:INTEL64${RB}")
                    endif()
                elseif(ARCH STREQUAL "skylake" OR ARCH STREQUAL "skylake-server" OR ARCH STREQUAL "skylake-server-avx512")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/arch:AVX2${RB}")
                    if(CMAKE_SIZEOF_VOID_P EQUAL 8) # compiling for x64
                        target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/favor:INTEL64${RB}")
                    endif()
                elseif(ARCH STREQUAL "knl")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/arch:AVX2${RB}" "${LB}/favor:ATOM${RB}")
                else()
            ...
            elseif(CMAKE_C_COMPILER MATCHES "icc.*$") # Intel compiler
                ...
                if(ARCH STREQUAL "penryn")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}${ARCHARG}sse4.1${RB}" "${LB}${XARG}sse4.1${RB}")
                elseif(ARCH STREQUAL "skylake")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}${ARCHARG2}core-avx2${RB}" "${LB}${XARG}core-avx2${RB}")
                elseif(ARCH STREQUAL "skylake-server")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}${ARCHARG2}core-avx2${RB}" "${LB}${XARG}core-avx512${RB}")
                elseif(ARCH STREQUAL "skylake-server-avx512")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}${ARCHARG2}core-avx2${RB}" "${LB}${XARG}core-avx512${RB}" "${LB}${QARG}opt-zmm-usage${ASGN}high${RB}")
                elseif(ARCH STREQUAL "knl")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}${ARCHARG2}core-avx2${RB}" "${LB}${XARG}mic-avx512${RB}")
                else()
                    set(_SETTING_SET FALSE PARENT_SCOPE)
                endif()
            elseif(CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
                if(ARCH STREQUAL "penryn")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=core2${RB}" "${LB}-msse4.1${RB}")
                elseif(ARCH STREQUAL "skylake")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=skylake${RB}")
                elseif(ARCH STREQUAL "skylake-server")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=skylake-avx512${RB}")
                elseif(ARCH STREQUAL "skylake-server-avx512")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=skylake-avx512${RB}" "${LB}-mprefer-vector-width=512${RB}")
                elseif(ARCH STREQUAL "knl")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=knl${RB}" "${LB}-mprefer-vector-width=512${RB}")
                else()
                    set(_SETTING_SET FALSE PARENT_SCOPE)
                endif()
            elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
                if(ARCH STREQUAL "penryn")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=penryn${RB}")
                elseif(ARCH STREQUAL "skylake")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=skylake${RB}")
                elseif(ARCH STREQUAL "skylake-server")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-march=skylake-avx512${RB}")
        ...
        if(NOT MODEL STREQUAL "" AND NOT MODEL STREQUAL "default")
            ...
            if(MSVC)
                if(MODEL STREQUAL "strict" OR MODEL STREQUAL "consistent")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/fp:strict${RB}")
                elseif(MODEL STREQUAL "precise")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/fp:precise${RB}")
                elseif(MODEL STREQUAL "fast" OR MODEL STREQUAL "fastest")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/fp:fast${RB}")
                else()
                    set(_SETTING_SET FALSE PARENT_SCOPE)
                endif()
            elseif(CMAKE_C_COMPILER MATCHES "icc.*$") # Intel compiler
                ...
                if(_SETTING_SET)
                    if(WIN32)
                        target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}/fp:${MODELARG}${RB}")
                    else()
                        target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-fp-model${RB} ${LB}${MODELARG}${RB}") # TODO: does this work, or do we need two separate arguments?
                    endif()
                endif()
            elseif(CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
                if(MODEL STREQUAL "strict" OR MODEL STREQUAL "consistent")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-ffp-contract=off${RB}")
                elseif(MODEL STREQUAL "precise")
                    # default behavior; nothing to do
                elseif(MODEL STREQUAL "fast")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-funsafe-math-optimizations${RB}")
                elseif(MODEL STREQUAL "fastest")
                    target_compile_options(${TARGET_NAME} ${SCOPE} "${LB}-ffast-math${RB}")
                else()
                    set(_SETTING_SET FALSE PARENT_SCOPE)
                endif()
            elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
